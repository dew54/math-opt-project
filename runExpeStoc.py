...
def runExpeStochastic(data, upperTimeLimit, penalty):
    ...
    FL_i_k_ct = S_ICEP.addVars([(s, i, k, c, t) for s in range(num_s) for i in range(num_i) for k in range(num_k) for c in range(num_c) for t in range(num_t) ], vtype=gb.GRB.CONTINUOUS, name="flowEpsilon")
    ...
    S_ICEP.addConstrs(((gb.quicksum( zeta[s, i, h, b].cost *  W_i_1_hb[s, i, h, b]  for h in range(num_h) for b in range(num_b)) 
            + 
                gb.quicksum(gamma[s, i, k, b, c].cost *  X_i_k_bc[s, i, k, b, c]  for k in range(num_k) for b in range(num_b) for c in range(num_c))
            +
                gb.quicksum(delta[s, i, k, c, b].cost *  Y_i_k_cb[s, i, k, c, b] for k in range(num_k) for c in range(num_c) for b in range(num_b))
            +
            ...
            +
                gb.quicksum(resources[s][i].loadingTime *  Y_i_k_cb[s, i, k, c, b] for k in range(num_k) for c in range(num_c) for b in range(num_b))
            +
                gb.quicksum(resources[s][i].unloadingTime *  X_i_k_bc[s, i, k, b, c] for k in range(num_k) for b in range(num_b) for c in range(num_c))
    ...
    # Eq. 5
    S_ICEP.addConstrs((FL_i_k_bc[s, i, k, b, c] <= resources[s][i].capacity * X_i_k_bc[s, i, k, b, c] for s in range(num_s) for i in range(num_i) for k in range(num_k) for b in range(num_b) for c in range(num_c) ), name="Eq-5")
    ...
    S_ICEP.addConstrs((gb.quicksum( FL_i_k_ab[s, i, k, a, b] for a in range(num_a)  ) 
        ==
            ...
            for s in range(num_s) for i in range(num_i) for k in range(num_k) for b in range(num_b))     
        , name="Eq-7")
    ...
    S_ICEP.addConstrs(((gb.quicksum(FL_i_k_bc[s, i, k, b, c]  for b in range(num_b) )
        ==
            ...
            for s in range(num_s) for i in range(num_i) for k in range(num_k) for c in range(num_c) for t in range(num_t)), name="Eq-8")
 
    # Eq. 12
    S_ICEP.addConstrs((((gb.quicksum(W_i_1_hb[s, i, h, b]  for h in range(num_h)) == gb.quicksum(X_i_k_bc[s, i, k, b, c]  for c in range(num_c))) for s in range(num_s) for i in range(num_i) for k in range(num_k) if k == 0 for b in range(num_b) )), name="Eq-12")

    # Eq. 13
    S_ICEP.addConstrs(((gb.quicksum(Y_i_k_cb[s, i, k - 1, c, b] for c in range(num_c)) == gb.quicksum(X_i_k_bc[s, i, k, b, c] for c in range(num_c) )) for s in range(num_s) for i in range(num_i) for k in range(num_k)  for b in range(num_b) if k!=0 ), name="Eq-13")

    # Eq. 14
    S_ICEP.addConstrs(((gb.quicksum(X_i_k_bc[s, i, k, b, c] for b in range(num_b)) >= gb.quicksum(Y_i_k_cb[s, i, k, c, b] for b in range(num_b))) for s in range(num_s) for i in range(num_i) for k in range(num_k) for c in range(num_c) if k!=num_k-1 ), name="Eq-14" ) # Altra condizione

    # Eq. 15
    S_ICEP.addConstrs((FL_a_t[s, a, t] >= 0 for s in range(num_s) for a in range(num_a) for t in range(num_t)), name="Eq-15")

    # Eq 16 
    S_ICEP.addConstrs((FL_i_k_ab[s, i, k, a, b] >= 0 for s in range(num_s) for i in range(num_i) for k in range(num_k) for a in range(num_a) for b in range(num_b)), name="Eq-16")

    # Eq. 17
    S_ICEP.addConstrs((FL_i_k_bc[s, i, k, b, c] >= 0 for s in range(num_s) for i in range(num_i) for k in range(num_k) for b in range(num_b) for c in range(num_c)), name="Eq-17")

    # Eq. 18
    S_ICEP.addConstrs((FL_i_k_ct[s, i, k, c, t] >= 0 for s in range(num_s) for i in range(num_i) for k in range(num_k) for c in range(num_c) for t in range(num_t)), name="Eq-18")
    ...
    # Eq. 30
    S_ICEP.addConstrs((FL_i_k_bc[s, i, k, b, c] <= resources[s][i].capacity*Z_i[i] for s in range(num_s) for i in range(num_i) for k in range(num_k) for b in range(num_b) for c in range(num_c)), name="Eq-30")
    
    # Eq. 31
    S_ICEP.addConstrs((scenarios[s].evaAreas[a].evaDemand == FL_a_t[s, a, 0] + gb.quicksum(FL_i_k_ab[s, i, k, a, b] for i in range(num_i) for k in range(num_k)  for b in range(num_b)) + N_a[s, a]  for s in range(num_s) for a in range(num_a)), name="Eq-31" )
    ...
    # Eq. 33
    S_ICEP.addConstrs((gb.quicksum( X_i_k_bc[s, i, k, b, c] for b in range(num_b) for c in range(num_c)) <= Z_i[i] for s in range(num_s) for i in range(num_i) for k in range(num_k)), name="Eq-33")

    # Eq. 34
    S_ICEP.addConstrs((gb.quicksum(Y_i_k_cb[s, i, k, c, b] for c in range(num_c) for b in range(num_b)) <= Z_i[i] for s in range(num_s) for i in range(num_i) for k in range(num_k) if k!=num_k-1 ), name="Eq-34")

    # Eq. 35
    S_ICEP.addConstrs(((N_a[s, a] >= 0 )for s in range(num_s) for a in range(num_a)), name="Eq-35")

    #S_ICEP.addConstrs(S_i[s, i] == Z_i[i] * S_i[s, i] for s in range(num_s) for i in range(num_i) )
    fraction = []
    ...
    for s in range(num_s):
        ...
        #divisor.append(sum(resources[s][i].fixedCost + resources[s][i].getVarCost(T) for i in range(num_i)))
    ...
    # Eq. for ensuring at least one resource is involved in operations
    #S_ICEP.addConstr(gb.quicksum(Z_i[i] for i in range(num_i)) >= 1)

    # Eq. for forcing m percent of population to be saved
    #S_ICEP.addConstrs((1-m) * sum(evaAreas[a].evaDemand for a in range(num_a)) >= gb.quicksum(N_a[s, a]  for a in range(num_a)) for s in range(num_s))


    obj1 = (((gb.quicksum(resources[s][i].fixedCost * Z_i[i] for i in range(num_i)))/(sum(resources[s][i].fixedCost + resources[s][i].getVarCost(T) for i in range(num_i))))) + (E_c)
    ...
    for s in range(num_s ):
        ...
        econ_1 = gb.quicksum(resources[s][i].fixedCost * Z_i[i] for i in range(num_i)) + scenarios[s].probability * (gb.quicksum(resources[s][i].getVarCost(S_i[s, i]) for i in range(num_i)) + (P*(gb.quicksum(N_a[s, a] for a in range(num_a) ))))
        econ_2 = gb.quicksum(resources[s][i].fixedCost * Z_i[i] for i in range(num_i)) + scenarios[s].probability * (gb.quicksum(resources[s][i].getVarCost(S_i[s, i]) for i in range(num_i)) + r[s]/T + (P*(gb.quicksum(N_a[s, a] for a in range(num_a) ))))
        S_ICEP.setObjectiveN(econ_2, s+1, 1)
    ...
    #S_ICEP.setObjectiveN(obj1, 0, 1, 1)
    #obj1 = gb.quicksum(r[s] for s in range(num_s))
    #S_ICEP.setObjective(obj1)
    #S_ICEP.setObjective(1)
